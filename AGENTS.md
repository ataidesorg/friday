...
Friday is a local-first agent harness in Go (module `github.com/ataidesorg/friday`) — a daily-driver terminal agent in the spirit of OpenCode / Grok Build / Hermes. This file is injected into every agent session working on this repo. [DESIGN.md](DESIGN.md) defines the TUI product and taste bar. `docs/adr/` holds design rulings.

Friday is pre-release. Public APIs, config, and persistence formats may still change.

## Hard rules — never violate

- Secrets never go into project config, logs, prompts, snapshots, Git, or telemetry payloads. Credentials live in env, the OS keychain, or Friday's encrypted secret store — never echo or print them.
...
- This is **Go**, not Rust. `go build ./cmd/friday`, never cargo.
...
| `just build` | `go build -o bin/friday ./cmd/friday` (`bin/` is gitignored) |
...
- `internal/skills`, `internal/commands` — skills (`skills/<name>/SKILL.md`) and slash commands (`.friday/commands/<name>.md`), read from the user home then the repository; a project entry replaces a user entry with the same name. There is no manifest, no install step, and no enablement state.
...
- `internal/workspace` — primary / ephemeral / worktree modes (worktrees under `<friday-home>/worktrees/<root-hash>/`).
- `internal/observability` — JSONL event trails, lazy sink, `friday trace` reader.
...
- `cmd/friday` — the process entrypoint and nothing else: `os.Exit(cli.Main())`.
...
- TUI code paths must keep plain-output parity: nothing may break `friday run` non-TTY output, and never write raw stderr from inside a live TUI turn (it tears the frame) — surface warnings as Warning events or in-band tool output.
- `.friday/local/` is gitignored (session state); `.friday/commands/` and `skills/` are committed.
